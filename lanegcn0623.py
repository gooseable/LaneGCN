...
class MapNet(nn.Module):
    ...
    def forward(self, graph:List[List[torch.Tensor]]):
        print("in mapnet")
        if (
            len(graph[get_graph_index("feats")]) == 0
            or len(graph_get(graph, 'pre', -1, "u")) == 0
            or len(graph_get(graph, "suc", -1, "u")) == 0
        ):
            temp = graph[get_graph_index("feats")]
            return (
                temp[0].new_empty(temp[0].shape).resize_(0),
                [temp[0].new_empty(temp[0].shape).long().resize_(0) for x in graph[get_graph_index("idcs")]],
                [temp[0].new_empty(temp[0].shape).resize_(0)],
            )

        feat = self.input(torch.cat(graph[get_graph_index("ctrs")], 0))
        feat += self.seg(graph[get_graph_index("feats")][0])
        feat = self.relu(feat)
                ...
                for i, ctr in enumerate(v):
                    ...
                    for key in self.fuse:
                        if key.startswith("pre") or key.startswith("suc"):
                            k1 = key[:3]
                            k2 = int(key[3:])
                            a_tmp = self.get_fuse_res(feat[graph_get(graph, k1, k2, 'v')], key, i)
                            '''
                            temp.index_add_(
                                0,
                            ...
                            '''
                            my_index_add(graph_get(graph, k1, k2, 'u'), temp, a_tmp)
                            

                    if len(graph[get_graph_index("left")][get_graph_index("u")] > 0):
                        tmp2 = self.get_fuse_res(feat[graph[get_graph_index("left")][get_graph_index("v")]], 'left', i)
                        '''
                        temp.index_add_(
                            0,
                        ...
                        '''
                        my_index_add(graph[get_graph_index("left")][get_graph_index("u")], temp, tmp2)
                        
                    if len(graph[get_graph_index("right")][get_graph_index("u")] > 0):
                        tmp3 = self.get_fuse_res(feat[graph[get_graph_index("right")][get_graph_index("v")]], 'right', i)
                        '''
                        temp.index_add_(
                            0,
                        ...
                        '''
                        my_index_add(graph[get_graph_index("right")][get_graph_index("u")], temp, tmp3)
                    ...
                    res = feat
        print("after mapnet")
        return feat, graph[get_graph_index("idcs")], graph[get_graph_index("ctrs")]
...
class A2M(nn.Module):
    ...
    def forward(self, feat: Tensor, graph: List[List[Tensor]], actors: Tensor, actor_idcs: List[Tensor], actor_ctrs: List[Tensor]) -> Tensor:
        """meta, static and dyn fuse using attention"""
        print("a2m forward")
        meta = torch.cat(
            (
                graph[get_graph_index("turn")][0],
                graph[get_graph_index("control")][0].unsqueeze(1),
                graph[get_graph_index("intersect")][0].unsqueeze(1),
            ),
            1,
        )
        ...
        for a_att in self.att:
            feat = a_att(
                feat,
                graph[get_graph_index("idcs")],
                graph[get_graph_index("ctrs")],
                actors,
                actor_idcs,
                actor_ctrs,
                self.th_,
...
class M2M(nn.Module):
    ...
    def forward(self, feat: Tensor, graph: List[List[Tensor]]) -> Tensor:
        """fuse map"""
        print("m2m forward")
        res = feat
        # for i in range(len(self.fuse["ctr"])):
                ...
                for i, a_v in enumerate(v):
                    ...
                    for key in self.fuse:
                        if key.startswith("pre") or key.startswith("suc"):
                            k1 = key[:3]
                            k2 = int(key[3:])
                            tmp = self.get_fuse_res(key, i, feat[graph_get(graph, k1, k2, 'v')])
                            '''
                            temp.index_add_(
                                0,
                                graph_get(graph, k1, k2, "u"),
                            ...
                            '''
                            my_index_add(graph_get(graph, k1, k2, "u"), temp, tmp)
                            

                    if len(graph[get_graph_index("left")][get_graph_index("u")] > 0):
                        tmp = self.get_fuse_res('left', i, feat[graph[get_graph_index('left')][get_graph_index('v')]])
                        '''
                        temp.index_add_(
                            0,
                            graph[get_graph_index("left")][get_graph_index("u")],
                        ...
                        '''
                        my_index_add(graph[get_graph_index("left")][get_graph_index("u")], temp, tmp)
                        
                    if len(graph[get_graph_index("right")][get_graph_index("u")] > 0):
                        tmp = self.get_fuse_res('right', i, feat[graph[get_graph_index("right")][get_graph_index("v")]])
                        '''
                        temp.index_add_(
                            0,
                            graph[get_graph_index("right")][get_graph_index("u")],
                        ...
                        '''
                        my_index_add(graph[get_graph_index("right")][get_graph_index("u")], temp, tmp)
...
class PredNet(nn.Module):
    ...
    def forward(self, actors: Tensor, actor_idcs: List[Tensor], actor_ctrs: List[Tensor]) -> Dict[str, List[Tensor]]:
        ...
        # return reg index warning but ok
