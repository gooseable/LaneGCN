...
class ArgoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if 'preprocess' in self.config and self.config['preprocess']:
            ...
            if self.train and self.config['rot_aug']:
                ...
                print(data['city'].size()) # float32
                print(data['orig'].size()) # float32
                print(data['gt_preds'].size())#float32
                print(data['has_preds']) # bool
                print(data['theta'].size()) # float64
                print(data['rot'].size()) # float32
                print(data['feats'].size()) # float32
                print(data['ctrs'].size()) # float32
                print(data['graph'].size())# dict类型
                #print(data['argo_id'].dtype) # int 32个id号
    ...
    def get_lane_graph(self, data):
        ...
        #graph = dict()
        data['graph_ctrs'] = np.concatenate(ctrs, 0)
        data['graph_num_nodes'] = num_nodes
        data['graph_feats'] = np.concatenate(feats, 0)
        data['graph_turn'] = np.concatenate(turn, 0)
        data['graph_control'] = np.concatenate(control, 0)
        data['graph_intersect'] = np.concatenate(intersect, 0)
        data['graph_pre'] = [pre]
        data['graph_suc'] = [suc]
        data['graph_lane_idcs'] = lane_idcs
        data['graph_pre_pairs'] = pre_pairs
        data['graph_suc_pairs'] = suc_pairs
        data['graph_left_pairs'] = left_pairs
        data['graph_right_pairs'] = right_pairs
        


        for k1 in ['graph_pre', 'graph_suc']:
            for k2 in ['u', 'v']:
                data[k1][0][k2] = np.asarray(data[k1][0][k2], np.int64)
        
        for key in ['graph_pre', 'graph_suc']:
            if 'scales' in self.config and self.config['scales']:
                #TODO: delete here
                data[key] += dilated_nbrs2(data[key][0], data['graph_num_nodes'], self.config['scales'])
            else:
                data[key] += dilated_nbrs(data[key][0], data['graph_num_nodes'], self.config['num_scales'])
    ...
    def __getitem__(self, idx):
        if 'preprocess' in self.config and self.config['preprocess']:
            ...
            if self.train and self.config['rot_aug']:
                ...
                # for i in range(15):
                #     print(data['graph'][i])
                # 添加graph中的10项 ['idcs', 'ctrs', 'feats', 'turn', 'control', 'intersect', 'pre', 'suc', 'left', 'right']):
                # graph',  
                new_data['graph_ctrs'] = ref_copy(data['graph']['ctrs'])
                new_data['graph_num_nodes'] = ref_copy(data['graph']['num_nodes'])
                new_data['graph_feats'] = ref_copy(data['graph']['feats'])
                new_data['graph_turn'] = ref_copy(data['graph']['turn'])
                new_data['graph_control'] = ref_copy(data['graph']['control'])
                new_data['graph_intersect'] = ref_copy(data['graph']['intersect'])
                new_data['graph_pre'] = ref_copy(data['graph']['pre'])
                new_data['graph_suc'] = ref_copy(data['graph']['suc'])
                new_data['graph_lane_idcs'] = ref_copy(data['graph']['lane_idcs'])
                new_data['graph_pre_pairs'] = ref_copy(data['graph']['pre_pairs'])
                new_data['graph_suc_pairs'] = ref_copy(data['graph']['suc_pairs'])
                new_data['graph_left_pairs'] = ref_copy(data['graph']['left_pairs'])
                new_data['graph_right_pairs'] = ref_copy(data['graph']['right_pairs'])
                new_data['graph_left'] = ref_copy(data['graph']['left'])
                new_data['graph_right'] = ref_copy(data['graph']['right'])
        ...
        data = self.read_argo_data(idx)
        data = self.get_obj_feats(data)
        data= self.get_lane_graph(data)

        data['idx'] = idx
        print('after get_lane_graph---------------------------------------')
        return data
