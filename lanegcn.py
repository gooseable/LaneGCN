...
config["test_split"] = os.path.join(root_path, "dataset/test_obs/tmp")
...
#         sum_temp = torch.zeros_like(B[j])
#         for i in range(len(temp)):       
#             sum_temp=torch.add(temp[i],sum_temp)
#         new_B += [sum_temp]
...
class Net(nn.Module):
    ...
    def forward(self, data:Tuple[List[Tensor], List[Tensor], List[Tensor], List[Tensor], 
        ...
        actor_ctrs = data[1] # list[15,2]
        # print('*' * 80)
        # print(actors)
        actors = self.actor_net(actors) #  onnx value pass   tensor[15,128]
        return actors
        ...
        graph_left = data[12]
        graph_right = data[13]
# nodes, node_idcs, node_ctrs 在MapNet中做了什么？
        ...
        actors = self.a2a(actors, actor_idcs, actor_ctrs) # not pass
...
def actor_gather(actors: List[Tensor]) -> Tuple[Tensor, List[Tensor]]:
    ...
    num_actors = [len(x) for x in actors] # [15]

    actors = [x.transpose(1, 2) for x in actors] # shape从list 15 20 3 变为list 15 3 20
    actors = torch.cat(actors, 0)# tensor 15 3 20

    actor_idcs = []
    count = 0
    for i in range(batch_size):
        idcs = torch.arange(count, count + num_actors[i]).to(actors.device)# 0 -14tensor cuda
...
class ActorNet(nn.Module):
    ...
    def forward(self, actors: Tensor) -> Tensor:
        ...
        outputs = []
        # 3次进入grp，每次grp执行两次Res1d
        for grp in self.groups:
            out = grp(out)
            outputs.append(out) # list[tensor[15,32,20],tensor[15,64,10],tensor[15,128,5]]

# 
        out = self.lateral[-1](outputs[-1]) # tensor[15,128,5]
        for i in range(len(outputs) - 2, -1, -1): # i
            # 上下采样 FPN
            out = F.interpolate(out, scale_factor=2.0, mode="linear", align_corners=False)
            # out += self.lateral[i](outputs[i])
            # a_lat:Conv1d
            for j, a_lat in enumerate(self.lateral): # j：0，1，2
                if i == j:
                    out += a_lat(outputs[i])

        out = self.output(out)[:, :, -1] # out Res1d->out ,tensor[15,128]
    ...
    def forward(self, idcs:List[torch.Tensor], ctrs:List[torch.Tensor], feats:List[torch.Tensor], turn:List[torch.Tensor], control:List[torch.Tensor],
       ...
       # return temp
                    ...
                    if len(left[0] > 0):
                        ...
                        '''
                        temp = my_index_add(left[0], temp, tmp2)#没变
                        #print('if len(left[0] > 0):')
                        #print(temp[0])
...
class PostProcess(nn.Module):
    ...
    def display(self, metrics, dt, epoch, lr=None):
        ...
        print()
        #return loss, cls, reg, ade1, fde1, ade, fde
